"""
first you have to understand what type of question and problem this code is solve
so go to this link (https://www.youtube.com/watch?v=3Q_oYDQ2whs)
and watch the video wich is explaining the question
...
## This code will:
## 1) mereging the two lists of schedule times
...
## This code will merege the two lists into one list means one or both of the persons are bussy in this time
...
def merge_poked_time(sch):
    nb = 0
    while nb < len(sch)-1:
        # geting the end-time for the first-block and the start-time for the second-block in the schedule
        end1 = sch[nb][1]
        start2 = sch[nb+1][0]
        ...
        # that means the two blocks of time are connected(it mens there is not free time between the two blocks)
        ...
        # so we can merege them 
            ...
            # that is mean we don't need for the whole second-block because the (period)-time of second-block is inside the (period)-time of the first-block
            # so we just delete the second-block
            if end2 < end1:
                sch.pop(nb+1)
                # and here we have to (minus) one1 from the index because after we got new block(we mereged the first one and the second one)
            ...
            # that's mean we can merege them because the end-time of the first block is crossing the start-time of the second one     
                ...
                # it's the same like the first (condetion) when we have the end-time and the start-time are similar to each other LOOK TO (@1)
                sch[nb][1] = end2
                sch.pop(nb+1)
                nb-=1
        nb+=1
...
def find_free_time(sch,workt1,workt2):
    ...
    for i in range(1441): # the day is 24 hours and each hour is 60 minute so we have 1440 minute in the day
        # Here i will check if this minute is between the work time for both persons 
        # if that is true that's mean this minute of the day is with in the working time the both persons 
        if (i >= str_to_min(workt1[0]) and i >= str_to_min(workt2[0])) and (i <= str_to_min(workt1[1]) and i <= str_to_min(workt2[1])):
            ...
            for tm1 in sch : # after that i have to check if this minute is within the time when one of the persons is bussy 
                if i > tm1[0] and i < tm1[1]: # if the minute is between the time when one of them are bussy
                    valid = False # i will not append it to the valid_minute list because it is not valid minute
            
            if valid: # if the valid is still True that mean this minute is valid and it's free time for both of the persons
                valid_minute.append(i) # so i will append it to the valid_minute list (free-time minute)
                # print(get_miltary_time_from_minute(i))
    return valid_minute

## To This point if you print the (valid_minute) list you will see a list of all the available minute to make meeting for both of them
# print(valid_minute)

##################################
## but if you want to convert this available and free minute to a readable format uncomment this code
...
## this function will just convert the results to a nice format
